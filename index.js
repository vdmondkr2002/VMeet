        ...
        if(connections[path].users){
            for(const user of connections[path].users){
                if(socket.id!=user.socketListId){
                    var ind = connections[path].users.findIndex(user=>user.socketListId===socket.id)
                    io.to(user.socketListId).emit("user-left", socket.id,connections[path].users[ind].name)
                }     
            }
